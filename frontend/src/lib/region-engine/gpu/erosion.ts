/**
 * 水力(管道模型,Mei et al. 2007)+ 热力侵蚀 —— 移植自 LAAS `Erosion.ts`。
 *
 * 网格:res² 格,texel = size/res。
 * 状态:地形 h、水 w、泥沙 s、四向出流通量 f(vec4 L,R,D,U)、流速 v(vec2)、
 * 硬度(静态)、沉积累积 depo。
 *
 * 每迭代 5 个 dispatch(WebGPU 保证 dispatch 顺序):
 *  1. flux:由水头差更新出流通量
 *  2. water:通量散度更新水量 + 降雨 − 蒸发;流速
 *  3. erode:容量 C = Kc·sin(slope)·|v| → 溶蚀/沉积(对抗硬度)
 *  4. advect:泥沙半拉格朗日回溯平流
 *  5. thermal:安息角崩塌(gather 形式,对称成对转移)
 *
 * 缓冲轮换:每个迭代边界高度都在 hA。
 */
...
import type { FloatBuffer, NB, NF, NI } from "./tsl-types";
...
// 参数标定自 LAAS(l≈2 m, dt 0.03);texel≈4 m 下保守可用。
const DT = 0.03;
const RAIN = 0.01;
const EVAP = 0.02;
const KC = 0.55;
...
export async function runErosion(
  ...
  // --- 初始化(拆分:每个 compute stage ≤8 个 storage buffer)---
  const initK1 = guard(() => {
    const { i } = cellXY();
  ...
  const makeHydra = (r: Roles): ComputeNode[] => {
    ...
    // 2. water + velocity(读 flux, wSrc → 写 wDst, vel)
    const waterK = guard(() => {
      const { x, y, i } = cellXY();
    ...
    // 3. erode/deposit(读 hA, sSrc, vel, wDst, hardness → 写 hB, sTmp)
    const erodeK = guard(() => {
      const { x, y, i } = cellXY();
    ...
    // 4. advect(半拉格朗日回溯,双线性;读 sTmp, vel → 写 sDst)
    const advectK = guard(() => {
      const { x, y, i } = cellXY();
  ...
  // 高度每迭代末都在 hA;水/泥沙在最后的 wDst/sDst
